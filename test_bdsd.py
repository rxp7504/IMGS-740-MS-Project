...
if __name__ == "__main__":
	...
	thermal = cv2.imread("_dataset/_ADAS_1_3/FLIR_00006_16bit.tiff",cv2.IMREAD_UNCHANGED) # raw thermal temperature image
	...
	thermal = crop_to_size(thermal,shape_hires)
	...
	ax[0].axis('off')
	ax[1].imshow(cv2.cvtColor(thermal,cv2.COLOR_BGR2RGB))
	ax[1].set_title(f"Thermal Image {thermal.shape}")
	ax[1].axis('off')
	plt.tight_layout()
	plt.show()
	...
	#warp_matrix = utils.create_registration_matrix(thermal, rgb, "_resources/adas_warp_matrix.npy")
	...
	utils.overlay_imgs(rgb_aligned,thermal)
